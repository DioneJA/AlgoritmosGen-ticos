...
public class GeneticAlgorithm {
    ...
    private final int quantChromo = 10;
    private int[][] result;
    private int range;
    private int[] assistFit = new int[200];
    private int[][] graph;
    
    public GeneticAlgorithm(int range,int[][] graph) {
        this.range = range;
    ...
    public int randomGen(int value) {
    ...
    public void genInitalPopulation() {
        int[] assist; //auxiliar
        int controller = 0;
        int firstNumber = randomGen(range);
        for (int i = 0; i < quantChromo; i++) {
            controller = 0;
    ...
    public int Roullet(){
        int total =0;
        for(int i=0;i<assistFit.length;i++){
            total+=assistFit[i];
        }
        
        int fitRandom = randomGen(total);
        System.out.print(fitRandom + " = ");
        int partialTotal = 0;
        int index = 0;
        
        while(partialTotal<=fitRandom){
            partialTotal += assistFit[index++];
        }
        System.out.println(partialTotal);
        return --index;
    }
    
    public void RoulletGeneration(){
        int newRange = (int) Math.floor(range*0.95);
        int[] aux = new int[newRange];
        int[] auxBoolean;
